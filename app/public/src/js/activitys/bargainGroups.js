...
var TOPICID_EXP = /bargainGroups\/\d*/ig;

var topicId = (function () {
	var ret = location.pathname.match(TOPICID_EXP)
	if (ret && ret[0]) {
		return ret[0].replace(/bargainGroups\//gi, '');
	}
	return '';
})();
/**
	...
	verifyHasBindMobel(function (data) {
		if (data.Code == 200) {
			...
			if (data.Data.HasBoundMobile) {
				openWin('/forymatouapp/activitys/bargainGroups/' + topicId + '/join');
			}
			else {
				//绑定手机
				verifyPhone();
	...
	verifyHasBindMobel(function (data) {
		if (data.Code == 200) {
			...
			if (data.Data.HasBoundMobile) {
				$.ajax({
					type: 'post',
					url: addAuth('/forymatouapp/activitys/bargainGroups/' + topicId + '/create'),
					success: function (result) {
						if (result.Code == 200) {
							remind.show('恭喜您，已成功创建团购');
							setTimeout(function () {
								openWin('/forymatouapp/activitys/bargainGroups/' + topicId);
							}, 15E2);
						}
						else {
...
/*		//支付页
	...
	$.ajax({
		type: 'get',
		url: '/forYmatouApp/activitys/bargainGroups/'+topicId+'/getProductInfos',
		success: function(result) {
			if (result.Code == 200) {
...
/*	$(document).on('click', '.pay-dialog .confirm', function(e) {
		...
		$.ajax({
			type: 'post',
			url: '/forymatouapp/activitys/bargainGroups/'+topicId+'/buy',
...
function getBuyInfo(productInfo) {
	...
	$.ajax({
		type: 'get',
		url: addAuth('/api/buyerInfo/' + productInfo.SellerId),
		success: function (result) {
			if (result.Code == 200) {
				if (result.Data) {
...
$('#J-join-code').on('input', function () {
	...
	if (_this.val().length == 5) {
		...
		$.ajax({
			type: 'post',
			url: addAuth(location.pathname + '/' + _this.val()),
			success: function (result) {
				...
				if (result.Code == 200) {
					...
					if (result.Data.Status != 0) {
						setTimeout(function () {
							openWin('/forYmatouApp/activitys/bargainGroups/' + _this.attr('data-topic-id') + param);
						}, 15E2);
					}
...
$('.J-share').on('click', function () {
	var _this = $(this),
		topicId = _this.attr('data-topic-id'),
		code = _this.attr('data-attr-code'),
		title = _this.attr('data-share-title'),
		picUrl = _this.attr('data-share-picurl'),
		tip = _this.attr('data-share-tip');
	window.location.href = [
		'/forYmatouApp/share?' + '&shareTitle=',
		encodeURIComponent(title),
		'&sharePicUrl=' + encodeURIComponent(picUrl),
		'&shareTip=' + encodeURIComponent(tip) + '&shareUrl=',
		encodeURIComponent('http://' + window.location.host + '/forymatouapp/activitys/bargainGroups/' + topicId + '/' + code + '/share'),
		'?shareFlag=1'
	].join('');
});
...
(function () {
	...
	$.ajax({
		type: 'get',
		url: addAuth('/forYmatouApp/activitys/bargainGroups/' + topicId + '/profile'),
