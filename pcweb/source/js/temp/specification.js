    ...
    $m.augment(spe, {
        ...
        _parseJSON: function () {
            ...
            $m.each(data.Spec, function (m, n) {
                ...
                $m.each(data.Data, function (i, j) {
                    var datail = data.Data[j].SpecDetail,
                        id = i.CatalogId;
                    that.length = datail.length
                    $m.each(datail, function (o, p) {
                        if (n == p) {
                            var sub = spe.sub = spe.sub || [],
                                map = spe.map = spe.map || {};
        ...
        _view: function (model, b) {
            var c = this.config,
                that = this,
                $ = window.$,
                con = $(this.container).closest(c.parent),
                ui, bool = false;
            ui = c.ui = c.ui || {
        ...
        _show: function (a, b) {
            var c = this.config.countdownnumber,
                that = this;
            if (c == 0) {
                a.show();
                b.hide();
        ...
        _control: function () {
            ...
            function handler(e) {
                var o = $(this),
                    map = $(o).attr('map'),
                    id = parseInt(map.slice(0, 1)),
                    s = argu[0],
                    that = argu[1],
                    unselete = c.spe[id + '']['map']['un'] && c.spe[id + '']['map']['un'][map + '-un'],
                    items = $(that.container).find('dl'),
                    diselement = items.find('a.' + c.disabled);
