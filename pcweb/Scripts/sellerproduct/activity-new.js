...
$("#pActivitySubmit").live('click', function () {
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.result == "success") {
                alert("报名成功");
                location.href = $("#hidSellerSiteUrl").val() + "/productsmanage";
            } else {
                alert("报名失败:" + result.msg);
            }
        }
...
/*撤销js*/
$("a.remove_action").click(function () {
    var productsInActivityId = $(this).closest("td").find("input[name=productsInActivityId]").val();
    $.ajax({
        contentType: 'application/json; charset=utf-8',
        dataType: 'json',
        url: "/Activity/DeleteProductsInActivity",
        data: JSON.stringify({ productsInActivityId: productsInActivityId }),
        type: 'post',
...
/*查看原因*/
$("a.view_reason_action").click(function () {
    var productsInActivityId = $(this).closest("td").find("input[name=productsInActivityId]").val();
    $.ajax({
        contentType: 'application/json; charset=utf-8',
        dataType: 'json',
        url: "/Activity/GetRejectReason",
        data: JSON.stringify({ productsInActivityId: productsInActivityId }),
        type: 'post',
        success: function (res) {
            if (res.result == "success") {
...
/*修改*/
$("a.update_action").click(function () {
    var productsInActivityId = $(this).closest("td").find("input[name=productsInActivityId]").val();
    //$.get('/Activity/PreUpdateProductInActivity?productsInActivityId=' + productsInActivityId, function (data) {
    //    $('#setActivityProductInner').empty();
    //    $('#setActivityProductInner').html(data);
    //    initCalendar();
    //    //显示浮层(支付状态）
    //    struc.alert('#setActivityProduct');
    //    $('#setActivityProduct .shut').click(function() {
    //        struc.close();
    //    });
    //});
    location.href = "/Activity/UpdateActivityProduct?productsInActivityId=" + productsInActivityId;
});
