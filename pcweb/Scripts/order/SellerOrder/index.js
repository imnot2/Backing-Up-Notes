...
j$(function () {
    ...
    j$('.buttonSellerAcceptOrder').click(function () {
        ...
        if (isInBlackList == "True") {
            if (confirm('该订单来自黑名单用户，您是否要接单？')) {F
                j$.post('/Order/SellerOrder/SellerAcceptOneOrder', postdata, function (data) {
                    if (data.success == '1') {
                        alert('确认接单成功');
    ...
    j$('#buttonSellerAcceptOrder').click(function () {
        ...
        j$.post('/Order/SellerOrder/SellerAcceptOrderBatch', postdata, function (data) {
            if (data.success == '1') {
                alert('确认接单成功');
                if (data.message != '') {
                    alert(data.message);
    ...
    j$('.buttonSellerRefuseOrder').click(function () {
        ...
        var divform = j$(this).closest('.divform').find('*');
        var postdata = j$.toJSON(divform.serializeObject());
        j$.post('/Order/SellerOrder/SellerRejectOrder', postdata, function (data) {
            if (data.success == '1') {
                alert('取消接单成功');
                window.location.href = data.redirect;
            }
    ...
    $('#btnSellerRejectUnpayedOrder').click(function () {
        ...
        if (confirm("您确定要批量取消这些订单吗？")) {
            j$.post('/Order/SellerOrder/SellerRejectUnpayedOrder', postdata, function (data) {
                if (data.success == '1') {
                    alert('取消接单成功');
                    window.location.href = data.redirect;
                }
...
j$(function () {
    ...
    $('.buttonSellerSkipPostPay').click(function () {
        ...
        if (confirm('您确定该笔订单不需要补款吗？')) {
            ...
            $.post('/Order/SellerOrderPostPay/Skip', $.toJSON({ 'OrderId': orderid, 'Reason': "默认" }), function (data) {
                if (data.success == '1') {
                    alert('操作成功！');
                    window.location.href = data.redirect;
                } else {
...
j$(document).ready(function () {
    ...
    $('.canNotDelayBtn1').click(function () {
        alert($(this).next().val());
    });
    ...
    $('.cancelPostPay').click(function () {
        var orderId = $(this).next().val();
        if (confirm('要撤销补款申请吗？')) {
            $.post('/Order/SellerOrderPostPay/SellerCancelPostPay', $.toJSON({ 'orderId': orderId }), function (data) {
                if (data.success == true) {
