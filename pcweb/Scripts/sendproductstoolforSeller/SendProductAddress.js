...
$(function () {
    $('#submitBtn').click(function () {
        ...
        $.ajax({
            url: "/SendProductsToolForSeller/GetSendProductsInfo",
            type: "POST",
            data: $.toJSON(params),
    ...
    $('.buttonProductDetail').click(function() {
        $('#formExportByProduct').empty();
        for (var i = 0; i < SendProductAddresses.length; i++) {
            var orderIds = [];
            for (var j = 0; j < SendProductAddresses[i].Orders.length; j++) {
                orderIds.push(SendProductAddresses[i].Orders[j].OrderId);
            }
            $('#formExportByProduct').append('<input type="hidden" name="orderIds" value="' + orderIds.join(",") + '" />');
        }
        $('#formExportByProduct').submit();
    });
});
