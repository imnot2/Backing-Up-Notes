define(function (require, exports, module) {
    ...
    */
    ...
    exports.fillOption = function (el, loc_id, selector, val ,fn) {
        var json = this.find(loc_id);
        if (json) {
            var i = 1, index = 1, option = '';
            $m.node.empty(el);
            ...
            $m.node.append(el, '<option>无</option>')
            $m.node.append(el, option);
            el.selectedIndex = index;
            $m.node.attr(el.options[index], 'selected', true)
            fn && fn(el)
        }
    }
    ...
    exports.init = function (o) {
        ...
        var wrap = $m.node(o.container)[0];
        var sel = $m.node("select", wrap);
        ...
        that.fillOption(sProvince, '0', o.s1, o.val,o.fn);
        dpv = getvalue(sProvince);
        
        that.fillOption(sCity, '0,' + dpv, o.s2, o.val, o.fn);
        dcv = getvalue(sCity);
                
        that.fillOption(sCounty, '0,' + dpv + ',' + dcv, o.s3, o.val, o.fn);
                
        $m.event.delegate('select', 'change', function (e) {
            if (e.target == sProvince) {
                dpv = getvalue(sProvince);
                dcv = that.fillOption(sCity, '0,' + dpv, o.s2, o.val, o.fn);
                dcv = getvalue(sCity);
                that.fillOption(sCounty, '0,' + dpv + ',' + dcv, o.s3, o.val, o.fn);
                return;
            }
