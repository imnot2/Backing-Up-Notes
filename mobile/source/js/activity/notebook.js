        ...
        globalFn: function() {
            $('.look-rules a').on('click', function() {
                var html = [
                    ...
                    '    <p>1、单笔订单金额满100元减30元</p>',
                    '    <p>2、仅限新注册用户使用，每个用户每个券码仅限使用1次(同一用户定义，收件人姓名、地址、手机号及同一部手机均视为同一用户)</p>',
                    '    <p>3、该优惠券仅限洋码头移动端使用（WAP/洋码头海外商城APP/洋码头扫货神器APP）,特价商品除外</p>',
                    '    <p>4、优惠券码一经使用，不可退回</p>',
                    '    <p>5、数量有限，先到先用</p>',
                    '    <div class="bottom-tips">活动最终解释权在法律范围内归洋码头所有</div>',
                ...
                $('.btn-confirm').on('click', function() {
                    ...
                    $.post('/Register/SubFenzhongRegist', d, function(res) {
                        if (res.success) {
                            ...
                            location.href = '/ActivityPage/BJB';
                        } else {
...
win.ac.router('/bjb', win.ac.fenzhong.register);
